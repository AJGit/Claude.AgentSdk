...
public static class Program
{
    ...
    private static void ShowInbox(MockEmailStore store)
    {
        ...
        var emails = store.GetInbox(15);
        foreach (var email in emails)
        {
            var readIcon = email.IsRead ? " " : "*";
            var starIcon = email.IsStarred ? "[*]" : "   ";
            var truncSubject = email.Subject.Length > 40
                ? email.Subject[..37] + "..."
                : email.Subject;
        ...
        var unread = emails.Count(e => !e.IsRead);
        Console.WriteLine();
        Console.ForegroundColor = ConsoleColor.DarkGray;
                ...
                foreach (var block in assistant.MessageContent.Content)
                {
                    switch (block)
                    {
                        ...
                        case ToolUseBlock toolUse:
                            Console.ForegroundColor = ConsoleColor.Yellow;
                            var toolName = toolUse.Name.Replace("mcp__email-tools__", "");
                            Console.Write($"\n[{toolName}");
    ...
    private static string? GetToolInputSummary(string toolName, JsonElement? input)
    {
        ...
        try
        {
            ...
            {
                "search_inbox" when element.TryGetProperty("query", out var q) =>
                    $"\"{Truncate(q.GetString(), 30)}\"",
                "read_emails" when element.TryGetProperty("ids", out var ids) =>
                    $"{ids.GetArrayLength()} email(s)",
                "mark_as_read" or "mark_as_unread" or "star_email" or "unstar_email" or "archive_email"
                    when element.TryGetProperty("ids", out var ids) =>
                    $"{ids.GetArrayLength()} email(s)",
                "add_label" or "remove_label" when element.TryGetProperty("label", out var l) =>
                    l.GetString(),
                "get_inbox" when element.TryGetProperty("limit", out var l) =>
                    $"limit={l.GetInt32()}",
                _ => null
            };
        }
